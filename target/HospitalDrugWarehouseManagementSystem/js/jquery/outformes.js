...
function getexcel() {
    window.location.href="outputForm_upload";
}
function send() {
    ...
    var stringDome = tableContext.join()
        $.ajax({
            async: false,
            url: 'outputForm_dealTable',
            type: 'POST',
            tradition: true,
            data: {'stringDome': stringDome}
        })
        window.location.href = "outputForm_dealTable?tableTime=" + $("#time").val() + "&tablePerson=" + $("#persongo").val() + "&tableWarehouse=" + $("#warehousego").val();
}